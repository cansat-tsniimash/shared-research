...
#define SD_CMD_TEAK_TIMEOUT (60000)
#define SD_BUSY_TEAK_TIMEOUT (60000)
...
{
   size_t timeout = SD_BUSY_TEAK_TIMEOUT;
   uint8_t buffer;
...
sd_error_t sd_block_write_multi(size_t offset, const void * block_start, size_t block_count)
{
   ...
   sd_cs(true);
   GOTO_END_IF_ERROR(sd_wait_busy());
   ...
   {
      if (i != 0)
         GOTO_END_IF_ERROR(sd_wait_busy()); // ждем пока карта освободиться
