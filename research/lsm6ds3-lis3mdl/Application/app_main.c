...
/*
 * Суть токова:
 * Этот проект - демка использования официальных стмикровских драйверов для гироскопа/акселерометра lsm6ds3
 * и магнетометра lis3mdl.
 *
 * Проект собирается для black pill черной платки и выводит данные с датчиков USART1 с TX на PA9
 * Однако есть нюанс. Поскольку у нас нет lis3mdl в чистом виде - мы заменяем его аналогом lsm303c.
 * У этого аналога есть несколько небольших отличий, которые обозначены в main-е комментариями,
 * но самое главное - это то, что этот магнетометр использует исключительно трехпроводной SPI.
 * Четвертого провода там просто напросто нет.
 *
 * Это определяет особенности соответствующих write_reg и read_reg функций, а так же
 * последовательность настройки обязательно начинается с включения трехпроводного режима
 */
